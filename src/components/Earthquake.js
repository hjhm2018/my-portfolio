...
export class Earthquake extends Component {
  ...
  onClick() {
    fetch(
      `https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=${this.state.startDate}&endtime=${this.state.endDate}&minmagnitude=${this.state.minMagnitude}&maxmagnitude=${this.state.maxMagnitude}`
    ).then((response) => {
      response
        .json()
        .then((data) => {
  ...
  render() {
    ...
    return (
      <div className="container-fluid text-center pt-md-4 pt-sm-1">
        ...
        <button className="btn btn-primary p-2" onClick={() => this.onClick()}>
          Search
        </button>
        ...
        <table className="table table-bordered table-dark text-center col-sm-8 rounded mx-auto mt-4 mb-5 pb-5">
          ...
          <tbody>
            {this.state.show && this.state.earthquakeData.features ? (
              this.state.earthquakeData.features
                .filter((data) =>
